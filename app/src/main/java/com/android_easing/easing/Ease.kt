...
interface Ease {
    fun calculate(x: Float): Double
}

/* Linear*/
class Linear : Ease {
    override fun calculate(x: Float) = x.toDouble()
}

/*Sine*/
class EaseInSine : Ease {
    override fun calculate(x: Float) = 1f - cos((x * PI) / 2f)
}

class EaseOutSine : Ease {
    override fun calculate(x: Float) = sin((x * PI) / 2f)
}

class EaseInOutSine : Ease {
    override fun calculate(x: Float) = -(cos(PI * x) - 1) / 2f
}

/*Elastic*/
class EaseInElastic : Ease {
    override fun calculate(x: Float) = when (x) {
        0f -> 0.0
        1f -> 1.0
        else -> {
            val c4 = (2 * PI) / 3
...
class EaseInOutElastic : Ease {
    override fun calculate(x: Float): Double {
        val c5 = (2 * PI) / 4.5
        return when {
            (x == 0f) -> 0.0
            (x == 1f) -> 1.0
